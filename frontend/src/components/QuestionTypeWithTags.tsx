...
export default function QuestionTypeWithTags({
  tags = [],
  label,
  deleteQuestion = () => {},
  setTags = () => {},
}: QuestionTypeWithTagsProps) {
  ...
  const deleteTag = (deleteTag: string) => {
    ...
    setTags(
      tags.slice(0, deleteTagIndex).concat(tags.slice(deleteTagIndex + 1))
    );
  };
  return (
    <div className="flex flex-col gap-2 bg-white border rounded-md p-2 w-full text-gray-500">
      <div className="flex justify-between">
        <h5 className="ml-2">{label}</h5>
        <button
          onClick={deleteQuestion}
          type="button"
          className="text-red-300 hover:bg-red-50 hover:text-red-400"
        >
      ...
      <hr />
      <Popover>
        <PopoverTrigger className="flex items-center gap-2 100 p-2 rounded-md text-sm hover:bg-gray-100">
