...
import { IconAlertTriangle, IconArrowLeft, IconArrowRight, IconArrowsRightLeft, IconCircle, IconInfoCircle, IconThumbUp, IconWand } from "@tabler/icons-react"
...
export default function InterviewInsights({questionAnswerPairs}: InterviewInsightsProps) {
  ...
  return <div className="flex flex-col items-center justify-between p-8 h-full">
    <div className="flex gap-8 w-full justify-center">
      <div className="max-w-lg w-full p-6 shadow-md bg-white rounded-lg flex flex-col gap-3">
        <div className="flex gap-2 items-end">
          <QuestionNumber questionNum={questionIndex + 1} totalQuestionNum={questionAnswerPairs.length}/>
          {
...
function InsightComponent({type, children}: InsightComponentProps) {
  ...
  return <div className="flex rounded-md overflow-hidden border items-center divide-x shadow">
    <div className={`shrink-0 p-2 w-fit h-full ${bgClasses}`}>
      {icon}
    </div>
    <div className="p-2">
      {children}
