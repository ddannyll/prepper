...
import { IconFilePencil, IconHome, IconMenu, IconMenu2, IconPlus, IconUser, IconX} from '@tabler/icons-react'
...
export default function Sidebar() {
  ...
  return <div>
    <button className='absolute z-20 md:hidden left-5 top-5' onClick={() => setSidebarOpen(!sidebarOpen)}>
      <IconMenu2 className={cn('w-7 h-7 text-blue-500', {
        "hidden": sidebarOpen
      })}/>
      <IconX className={cn('w-7 h-7 text-red-500', {
        "hidden": !sidebarOpen
      })} />
    </button>
    <div className={cn("z-10 absolute w-screen md:w-[224px] md:relative md:flex flex-col shrink-0 h-full  py-4 px-6 bg-white shadow ",
      {
        "hidden": !sidebarOpen,
        "pt-20": sidebarOpen
      }
      ...
      <ul className='flex flex-col gap-2 mt-8'>
        ...
        <SidebarButton href='/coverletter' onClick={() => setSidebarOpen(false)}>
          <IconFilePencil />
        Cover Letter
        </SidebarButton>
      </ul>
