...
export default function QuestionCard({
  ...
  totalQuestionNum,
  prompt,
  promptReadIndex = Infinity,
  tags,
}: QuestionCardProps) {
  return (
    <div
      className={`shadow-md border-gray-100 bg-white border container p-6 max-w-lg rounded-lg flex flex-col gap-3 ${className}`}
    >
      <div className="flex justify-between items-end">
        <QuestionNumber questionNum={questionNum} totalQuestionNum={totalQuestionNum} />
        <p className="text-sm italic text-gray-500 mr-2">generated by AI</p>
      </div>
      <hr className="-mx-6 border-y-2 border-blue-500" />
      <QuestionCardMainSection tags={tags} prompt={prompt} promptReadIndex={promptReadIndex} />
...
export function QuestionNumber({questionNum, totalQuestionNum}: QuestionNumberProps) {
...
interface QuestionCardMainSectionProps {
  tags: string[]
  prompt: string[]
  promptReadIndex?: number
}
export function QuestionCardMainSection({tags, prompt, promptReadIndex=Infinity}: QuestionCardMainSectionProps) {
  return <>
